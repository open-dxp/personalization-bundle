...
opendxp_personalization:
    targeting:           
        storage_id: OpenDxp\Bundle\PersonalizationBundle\Targeting\Storage\CookieStorage
```


## Implement a Custom Targeting Storage

Basically, a targeting storage is a class implementing the [`TargetingStorageInterface`](https://github.com/open-dxp/personalization-bundle/tree/1.x/src/Targeting/Storage/TargetingStorageInterface.php)
which is registered as service. Details how to handle data varies heavily on the underlying storage, but you can take the
[core storages](https://github.com/open-dxp/personalization-bundle/tree/1.x/src/Targeting/Storage) as starting point.


## Core Storage Implementations

OpenDxp implements different storage engines which each has its pros and cons. In general it is recommended to start with
...
To change the save handler, override the [service definition](https://github.com/open-dxp/personalization-bundle/tree/1.x/config/targeting.yaml#L24)
...
details to the service definition. An example is shipped with the [core service definitions](https://github.com/open-dxp/personalization-bundle/tree/1.x/config/targeting.yaml#L35).
...
opendxp_personalization:
    targeting:
        ...
        storage_id: OpenDxp\Bundle\PersonalizationBundle\Targeting\Storage\SessionStorage
```
...
* Might not work properly in conjunction with full page caches. OpenDxp's full page cache is disabled when the session
