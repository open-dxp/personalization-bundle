...
The following section describes the technical concepts and aspects of the OpenDxp targeting engine. For usage 
description and feature listing see your [user docs](../01_Usage/01_Concepts.md)
first. 
...
or using the cookie `opendxp_targeting_enabled=1`.
...
Set the path to the database file in your `parameters.yaml` to enable the geo support in OpenDxp: 
```yaml
opendxp.geoip.db_file: /usr/share/GeoIP/GeoLite2-City.mmdb
``` 
...
The logic defined above can be applied to any document. Documents which are loaded through OpenDxp's standard routing and
...
The toolbar is only shown if a `opendxp_targeting_debug` cookie exists and is set and its value evaluates to true. You can
...
* <a href="javascript:(function()%7Bdocument.cookie %3D 'opendxp_targeting_debug%3D1%3B path%3D%2F'%7D)()">Enable OpenDxp Targeting Toolbar</a> 

## Opt-in for Targeting

<div class="alert alert-warning">
A user has the possibility to enable targeting at any time, by setting the following cookie `opendxp_targeting_enabled=1` .
</div> 
