...
opendxp.registerNS("opendxp.bundle.personalization.document.areatoolbar");
/**
 * @private
 */
opendxp.bundle.personalization.document.areatoolbar = Class.create({
    ...
    addTargetingPanel: function (document , lbar) {
        ...
        if (opendxp.globalmanager.get("target_group_store").getCount() === 0) {
            return;
        }
        ...
        var cleanupFunction = function () {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_personalization_clear_targeting_page_editable_data'),
                method: "PUT",
                params: {
        ...
        var docEdit = opendxp.globalmanager.get("document_" + document.id).edit;
        ...
        this.targetGroupStore = Ext.create('Ext.data.JsonStore', {
            proxy: {
                type: 'ajax',
                url: Routing.generate('opendxp_bundle_personalization_targeting_targetgrouplist', {'add-default': true})
            },
            fields: ["id", "text"],
            listeners: {
                load: function () {
    ...
    updateTargetGroupText: function (targetgroup) {
        ...
        if (record) {
            this.targetGroupText.update('&nbsp;&nbsp;<img src="/bundles/opendxpadmin/img/flat-color-icons/manager.svg" style="height: 16px;" align="absbottom" />&nbsp;&nbsp;'
                + record.data.text);
        } else {
