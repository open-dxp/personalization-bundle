...
opendxp.registerNS("opendxp.bundle.personalization.settings.rules.item");
/**
 * @private
 */
opendxp.bundle.personalization.settings.rules.item = Class.create({
    initialize: function(parent, data) {
        ...
        this.tabPanel = new Ext.TabPanel({
            ...
            forceLayout: true,
            id: "opendxp_targeting_panel_" + this.data.id,
            buttons: [{
                text: t("save"),
    ...
    initializeConditions: function() {
        ...
        if (this.data.conditions && this.data.conditions.length > 0) {
            for (var i = 0; i < this.data.conditions.length; i++) {
                try {
                    condition = opendxp.bundle.personalization.settings.conditions.create(this.data.conditions[i].type);
                } catch (e) {
                    console.error(e);
                    continue;
                }
    ...
    initializeActions: function() {
        ...
        if (this.data.actions && this.data.actions.length > 0) {
            for (var i = 0; i < this.data.actions.length; i++) {
                try {
                    action = opendxp.bundle.personalization.settings.actions.create(this.data.actions[i].type);
                } catch (e) {
                    console.error(e);
                    continue;
                }
    ...
    getConditions: function() {
        ...
        var addMenu = [];
        Ext.Array.forEach(opendxp.bundle.personalization.settings.conditions.getKeys(), function(key) {
            var condition;

            try {
                condition = opendxp.bundle.personalization.settings.conditions.create(key);
            } catch (e) {
                console.error(e);
                return;
            }
    ...
    getActions: function () {
        ...
        var addMenu = [];
        Ext.Array.forEach(opendxp.bundle.personalization.settings.actions.getKeys(), function(key) {
            var action;

            try {
                action = opendxp.bundle.personalization.settings.actions.create(key);
            } catch (e) {
                console.error(e);
                return;
            }
    ...
    save: function () {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_bundle_personalization_targeting_rulesave'),
            method: 'PUT',
            params: {
                id: this.data.id,
