...
opendxp.registerNS("opendxp.bundle.personalization.settings.rules.panel");
/**
 * @private
 */
opendxp.bundle.personalization.settings.rules.panel= Class.create({

    initialize: function() {
        this.treeDataUrl = Routing.generate('opendxp_bundle_personalization_targeting_rulelist');
    },
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            this.layout = new Ext.Panel({
                ...
                closable: true,
                border: false,
                iconCls: "opendxp_icon_targeting",
                items: [
                    this.getTree(),
                    this.getTabPanel()
                ]
        ...
        if (!this.tree) {
            this.saveButton = new Ext.Button({
                ...
                handler: function () {
                    ...
                    // save order
                    Ext.Ajax.request({
                        url: Routing.generate('opendxp_bundle_personalization_targeting_ruleorder'),
                        params: {
    ...
    getTreeNodeListeners: function () {
        var treeNodeListeners = {
            ...
            'beforeitemappend': function (thisNode, newChildNode, index, eOpts) {
                var classes = [];
                var iconClasses = ['opendxp_icon_targeting'];
    ...
    addTargetComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 2 && regresult == value) {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_personalization_targeting_ruleadd'),
                method: 'POST',
                params: {
                    name: value
                },
    ...
    deleteTarget: function (tree, record) {
        opendxp.helpers.deleteConfirm(t('global_targeting_rule'), record.data.text, function () {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_personalization_targeting_ruledelete'),
                method: 'DELETE',
                params: {
                    id: record.data.id
                },
    ...
    openTarget: function (node) {
        ...
        var existingPanel = Ext.getCmp("opendxp_targeting_panel_" + node);
        if(existingPanel) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_bundle_personalization_targeting_ruleget'),
            params: {
                id: node
            },
            success: function (response) {
                try {
                    ...
                    var item = new opendxp.bundle.personalization.settings.rules.item(this, res);
                } catch (e) {
                    console.log(e);
                }
            }.bind(this)
