...
opendxp.registerNS("opendxp.bundle.personalization.settings.targetGroups.panel");
/**
 * @private
 */
opendxp.bundle.personalization.settings.targetGroups.panel = Class.create({

    initialize: function() {
        this.treeDataUrl = Routing.generate('opendxp_bundle_personalization_targeting_targetgrouplist');
    },
    ...
    getLayout: function () {
        ...
        if (this.layout == null) {
            this.layout = new Ext.Panel({
                ...
                closable: true,
                border: false,
                iconCls: "opendxp_icon_target_groups",
    ...
    getTreeNodeListeners: function () {
        var treeNodeListeners = {
            ...
            'beforeitemappend': function (thisNode, newChildNode, index, eOpts) {
                var classes = [];
                var iconClasses = ['opendxp_icon_target_groups'];

                if (!newChildNode.data.active) {
                    classes.push('opendxp_unpublished');
                }
    ...
    addTargetGroupComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 2) {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_personalization_targeting_targetgroupadd'),
                method: 'POST',
                params: {
                    name: value
                },
                success: function (response) {
                    ...
                    if(!data || !data.success) {
                        ...
                        opendxp.globalmanager.get("target_group_store").reload();
                    }
                }.bind(this)
            });
    ...
    deleteTargetGroup: function (tree, record) {
        opendxp.helpers.deleteConfirm(t('target_group'), record.data.text, function () {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_personalization_targeting_targetgroupdelete'),
                method: 'DELETE',
                params: {
                    id: record.data.id
                },
    ...
    openTargetGroup: function (node) {
        ...
        var existingPanel = Ext.getCmp("opendxp_target_groups_panel_" + node);
        if(existingPanel) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_bundle_personalization_targeting_targetgroupget'),
            params: {
                id: node
            },
            success: function (response) {
                var res = Ext.decode(response.responseText);
                var item = new opendxp.bundle.personalization.settings.targetGroups.item(this, res);
            }.bind(this)
        });
