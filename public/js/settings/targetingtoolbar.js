...
opendxp.registerNS("opendxp.bundle.personalization.settings.targetingtoolbar");
/**
 * @private
 */
opendxp.bundle.personalization.settings.targetingtoolbar = Class.create({
    initialize: function () {
        var that = this;
        var cookieName = "opendxp_targeting_debug";

        var buttons = [];
        if ("1" === Ext.util.Cookies.get(cookieName)) {
            buttons.push({
                text: t("deactivate"),
                iconCls: "opendxp_icon_targeting_toolbar_disable",
                handler: function () {
            ...
            buttons.push({
                text: t("activate"),
                iconCls: "opendxp_icon_targeting_toolbar_enable",
                handler: function () {
